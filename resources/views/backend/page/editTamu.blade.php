    ...
    <section>
        ...
        <div class="container ps-5 pe-5">
            @if ($errors->any())
                ...
                @endforeach
            @endif
            <form action="/admin/tamu/edit/{{ $tamu->id }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <div class="mb-3">
                    <label for="exampleInputEmail1" class="form-label">Deskripsi :</label>
                    <textarea class="form-control" name="deskripsi" id="exampleInputEmail1" aria-describedby="emailHelp">{{ $tamu->deskripsi }}</textarea>
                ...
                <input type="hidden" name="imageLama" value="{{ $tamu->image }}">
